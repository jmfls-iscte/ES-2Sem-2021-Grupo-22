...
public class GuiQualidadeRegras extends Composite {
	...
	public GuiQualidadeRegras(Composite parent, int style, ProjMainGui mainWindow) {
		...
		lblNewLabel_2.setText("AvaliaÃ§Ã£o AutomÃ¡tica (recomendado)");

		Label lblNewLabel_1 = new Label(composite, SWT.NONE);
		lblNewLabel_1.setEnabled(false);
		lblNewLabel_1.setText("AvaliaÃ§Ã£o Manual");
		...
		// composite_2_2.setForeground(new org.eclipse.swt.graphics.Color(getDisplay(),
		// 0, 255, 128));
			...
			for (int i = 0; i < packages.size(); i++) {
				...
				for (int j = 0; j < packages.get(i).getClasslst().size(); j++) {
					...
					for (int k = 0; k < packages.get(i).getClasslst().get(j).getMethodslst().size(); k++) {
						...
						ti3.setText(0, packages.get(i).getClasslst().get(j).getMethodslst().get(k).getMethodEval()
								.getName_method());
						...
						for (String rule : packages.get(i).getClasslst().get(j).getMethodslst().get(k)
								.getCodesmelssEvaluator().keySet()) {
							int colindex = getColIndex(getColunas(mainWindow.getRules()), rule);
							if (colindex != -1) {
								ti3.setText(colindex, packages.get(i).getClasslst().get(j).getMethodslst().get(k)
										.getEvalByName(rule).toString());
							}
						}
					}
				}
